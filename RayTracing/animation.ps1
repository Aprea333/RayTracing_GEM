...
for (($angle = 0); $angle -lt $max_angle; $angle+=$step)
{
    ...
    dotnet run -- demo --angle_deg $angle --width 640 --height 480  
    dotnet run -- pfm2png --input_file ".\image.pfm" --output_file "output\image_$angleNNN.png"
}

ffmpeg -r 20 -s 640x480 -f image2 -i .\output\image_%03d.png -vcodec libx264 -pix_fmt yuv420p "output\spheres-perspective.mp4"
